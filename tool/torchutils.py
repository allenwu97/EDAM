...
class PolyOptimizer(torch.optim.SGD):
    ...
    def __init__(self, params, lr, weight_decay, max_step, warm_up_step):
        super().__init__(params, lr, weight_decay=weight_decay, momentum=0.9)
        ...
        print(self.__initial_lr)


    def step(self, closure=None):

        if self.global_step<self.warm_up_step:
            lr_mult = 0.1 + (self.global_step/self.warm_up_step)*0.9

        elif self.global_step < self.max_step:
            lr_mult = (1 - (self.global_step - self.warm_up_step) / (self.max_step - self.warm_up_step)) ** 0.9
